...
class DeltaChat extends EventEmitter {
  ...
  addDeviceMessage (chatId, msg) {
    debug(`addDeviceMessage ${chatId}`)
    if (!msg) {
    ...
    return binding.dcn_add_device_msg(this.dcn_context, Number(chatId), msg.dc_msg)
  }

  addDeviceMessageOnce (chatId, label, msg) {
    debug(`addDeviceMessageOnce ${chatId} ${label}`)
    if (!msg) {
      throw new Error('invalid msg parameter')
    }
    if (typeof msg === 'string') {
    ...
    return binding.dcn_add_device_msg(this.dcn_context, Number(chatId), String(label), msg.dc_msg)
  }
  ...
  getChatInfoJSON (chatId) {
    debug(`getChatInfoJSON ${chatId}`)
    return binding.dcn_chat_get_info_json(this.dcn_context, Number(chatId))
  }
